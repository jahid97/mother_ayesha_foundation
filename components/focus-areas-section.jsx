...
import { Stethoscope, Eye, BookOpen, FlaskConical, Home, LifeBuoy, Rocket } from "lucide-react"
...
const areas = [
  ...
  { icon: Rocket,       bg: "bg-[#f3e8fd]", hoverBg: "group-hover:bg-[#e9d5fb]", iconColor: "text-[#9c27b0]", label: "Entrepreneurship & Social Business", description: "Incubating social businesses & women/youth entrepreneurs", href: "/social-business" },
    ...
    <section className="bg-white py-16">
      <div className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-7 gap-6 text-center">
          {areas.map(({ icon: Icon, bg, hoverBg, iconColor, label, description, href }) => {
            const inner = (
              <>
            ...
            return href ? (
              <Link key={label} href={href} className="flex flex-col items-center group cursor-pointer">
                {inner}
              </Link>
            ) : (
